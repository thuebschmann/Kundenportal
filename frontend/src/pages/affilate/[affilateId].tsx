...
const EditAffilate = () => {
  ...
  const { affilateId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: affilateId }));
  }, [affilateId]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: affilateId, data }));
...
EditAffilate.getLayout = function getLayout(page: ReactElement) {
  return <LayoutAuthenticated>{page}</LayoutAuthenticated>;
};

export default EditAffilate;
