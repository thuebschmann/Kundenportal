...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/projekt/autocomplete`, { params });
  return response.data;
}

const ProjektSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.projekt.hasPermissionToCreate,
});

export default connect(select)(ProjektSelectItem);
