...
// eslint-disable-next-line
import * as kundeDataFormat from 'pages/CRUD/Kunde/table/KundeDataFormatters';

import actions from 'actions/projekt/projektListActions';
import actionsProjekt from 'actions/projekt/projektFormActions';
...
const ProjektTable = () => {
  ...
  const count = useSelector((store) => store.projekt.list.count);
  const modalOpen = useSelector((store) => store.projekt.list.modalOpen);
  const rows = useSelector((store) => store.projekt.list.rows);
  const idToDelete = useSelector((store) => store.projekt.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsProjekt.doUpdate(id, data));
  };
  ...
  const sendProjektCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('projekt/bulk-import', data, {
        headers: {
  ...
  const columns = [
...

    {
      ...
      valueFormatter: ({ value }) =>
        dataFormat.kundeManyListFormatter(value).join(', '),
  ...
  return (
    <div>
      <Widget title={humanize('Projekt')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/projekt/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Kunde'
                  : 'http://localhost:8080/api-docs/#/Kunde'
              }
            >
              API documentation for projekt
            </LinkMaterial>
          </div>
        }
      </Widget>
