...
function kundeManyListFormatter(val) {
  if (!val || !val.length) return [];
  return val.map((item) => item.id);
}
function kundeOneListFormatter(val) {
  if (!val) return '';
  return val.id;
}
function kundeManyListFormatterEdit(val) {
  if (!val || !val.length) return [];
  return val.map((item) => {
    return { id: item.id, label: item.id };
  });
}
function kundeOneListFormatterEdit(val) {
  if (!val) return '';
...
export {
  ...
  kundeOneListFormatterEdit,
  kundeManyListFormatterEdit,
  kundeOneListFormatter,
  kundeManyListFormatter,
};
