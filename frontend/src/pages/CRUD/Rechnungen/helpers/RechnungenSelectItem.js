...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/rechnungen/autocomplete`, { params });
  return response.data;
}

const RechnungenSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.rechnungen.hasPermissionToCreate,
});

export default connect(select)(RechnungenSelectItem);
