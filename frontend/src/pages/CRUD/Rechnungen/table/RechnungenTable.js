...
import * as dataFormat from 'pages/CRUD/Rechnungen/table/RechnungenDataFormatters';

import actions from 'actions/rechnungen/rechnungenListActions';
import actionsRechnungen from 'actions/rechnungen/rechnungenFormActions';
...
const RechnungenTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'invoice_id', title: 'invoice_id', number: 'true' },
    { label: 'projekt_id', title: 'projekt_id', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.rechnungen.list.count);
  const modalOpen = useSelector((store) => store.rechnungen.list.modalOpen);
  const rows = useSelector((store) => store.rechnungen.list.rows);
  const idToDelete = useSelector((store) => store.rechnungen.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsRechnungen.doUpdate(id, data));
  };
  ...
  const sendRechnungenCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('rechnungen/bulk-import', data, {
        headers: {
  ...
  const getRechnungenCSV = async () => {
    const response = await axios({
      url: '/rechnungen?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'rechnungenCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Rechnungen')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/rechnungen/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Rechnungen'
                  : 'http://localhost:8080/api-docs/#/Rechnungen'
              }
            >
              API documentation for rechnungen
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default RechnungenTable;
