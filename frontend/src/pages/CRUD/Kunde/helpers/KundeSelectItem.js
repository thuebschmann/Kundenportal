...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/kunde/autocomplete`, { params });
  return response.data;
}

const KundeSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.kunde.hasPermissionToCreate,
});

export default connect(select)(KundeSelectItem);
