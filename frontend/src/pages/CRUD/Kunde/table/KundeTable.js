...
import actions from 'actions/kunde/kundeListActions';
import actionsKunde from 'actions/kunde/kundeFormActions';
...
const KundeTable = () => {
  ...
  const count = useSelector((store) => store.kunde.list.count);
  const modalOpen = useSelector((store) => store.kunde.list.modalOpen);
  const rows = useSelector((store) => store.kunde.list.rows);
  const idToDelete = useSelector((store) => store.kunde.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsKunde.doUpdate(id, data));
  };
  ...
  const columns = [
...

    {
      ...
      valueFormatter: ({ value }) =>
        dataFormat.usersManyListFormatter(value).join(', '),
  ...
  return (
    <div>
      <Widget title={humanize('Kunde')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/kunde/new' className={classes.element}>
