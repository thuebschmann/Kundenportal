...
const EditRechnungen = () => {
  ...
  const { rechnungenId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: rechnungenId }));
  }, [rechnungenId]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: rechnungenId, data }));
...
EditRechnungen.getLayout = function getLayout(page: ReactElement) {
  return <LayoutAuthenticated>{page}</LayoutAuthenticated>;
};

export default EditRechnungen;
