...
class UsersRepository extends BaseRepository implements UsersRepositoryInterface
{
    ...
    public function findAll($params) : array
    {
        ...
        if (isset($params['filter'])) {
            ...
            if (isset($filter['idusersRange'])) {
                [$start, $end] = $filter['idusersRange'];

                if (!empty($start)) {
                    $query->where('idusers', '>=', $start);
                }

                if (!empty($end)) {
                    $query->where('idusers', '<=', $end);
                }
            }

            if (isset($filter['registratedatRange'])) {
                [$start, $end] = $filter['registratedatRange'];

                if (!empty($start)) {
                    $query->where('registratedat', '>=', $start);
                }

                if (!empty($end)) {
                    $query->where('registratedat', '<=', $end);
                }
            }
...
,
                    'idusers' => $attributes['idusers'] ?? null
,
                    'registratedat' => $attributes['registratedat'] ?? null
,
...
,
    ...
    public function findAllAutocomplete(array $params)
    {
        ...
        $query->select('*', 'lastName as label');

        if (isset($params['query'])) {
            $query->where('lastName', 'like', '%'.$params['query'].'%');
        }
